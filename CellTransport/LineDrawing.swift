...
extension SCNGeometry {
    class func lineThrough(points: [SCNVector3], width: Int = 20, closed: Bool = false, color: CGColor = UIColor.black.cgColor, mitter: Bool = false) -> SCNGeometry {
        ...
        let floatPoints = vertices.map { vector_float3($0) }
        geometry.setValue(NSData(bytes: floatPoints, length: MemoryLayout<vector_float3>.size * floatPoints.count), forKeyPath: "vertices")
